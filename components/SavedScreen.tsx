...
const SavedScreen = ({ items, onDelete, onOpen, onClearAll, onImport, notify }: Props) => {
  ...
  useEffect(() => {
    ...
    const close = (event: globalThis.KeyboardEvent) => { if (event.key === 'Escape') setSelected([]); };
  ...
  useEffect(() => {
    ...
    const close = (event: globalThis.KeyboardEvent) => { if (event.key === 'Escape') setConfirmClear(false); };
    window.addEventListener('keydown', close);
    return () => window.removeEventListener('keydown', close);
  }, [confirmClear]);
  ...
  const exportCsv = () => {
    ...
    download('napiyo-projects.csv', rows.map((row) => row.map(safeCsvCell).join(',')).join('\n'), 'text/csv;charset=utf-8');
  };
  ...
  const importProjects = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = '';
    if (!file) return;
    if (file.size > MAX_IMPORT_BYTES) return notify('That backup is too large. Use a JSON file smaller than 8 MB.');
    try {
      ...
      if (Array.isArray(raw) && raw.length > MAX_IMPORT_PROJECTS) return notify(`A backup may contain at most ${MAX_IMPORT_PROJECTS} projects.`);
  ...
  const exportGeo = (item: SavedItem, kind: 'geojson' | 'kml' | 'gpx') => {
    ...
    download(`${safeDownloadName(item.title)}.${kind}`, values[kind][0], values[kind][1]);
  };
  ...
  const printReport = (item: SavedItem) => {
    ...
    const area = escapeHtml(`${formatDecimal(item.sqFt)} sq ft · ${formatDecimal(item.sqM)} m²`);
    const hill = escapeHtml(formatHillsWords(item.sqFt));
    const terai = escapeHtml(formatTeraiWords(item.sqFt));
    const perimeter = escapeHtml(item.source?.perimeterFt ? `${formatDecimal(item.source.perimeterFt)} ft` : 'Not recorded');
    const savedDate = escapeHtml(new Date(item.date).toLocaleString());
    const generatedDate = escapeHtml(new Date().toLocaleDateString());
    report.document.open();
    report.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="referrer" content="no-referrer"><title>${title}</title><style>body{font-family:system-ui,sans-serif;margin:48px;color:#15231d}h1{font-size:30px}table{border-collapse:collapse;width:100%;margin-top:24px}td{border-bottom:1px solid #ddd;padding:12px 0}td:first-child{font-weight:700;width:35%}.note{margin-top:30px;padding:16px;background:#fff7ed;border:1px solid #fed7aa}@media print{button{display:none}}</style></head><body><h1>${title}</h1><p>Generated by Napiyo on ${generatedDate}</p><table><tr><td>Project type</td><td>${type}</td></tr><tr><td>Area</td><td>${area}</td></tr><tr><td>Hill system</td><td>${hill}</td></tr><tr><td>Terai system</td><td>${terai}</td></tr><tr><td>Perimeter</td><td>${perimeter}</td></tr><tr><td>Saved</td><td>${savedDate}</td></tr></table><p class="note">Planning aid only. This report is not a cadastral record, legal survey, valuation, or construction approval.</p><button type="button" onclick="window.print()">Print or save as PDF</button></body></html>`);
    report.document.close();
  };

  const trapFocus = (event: KeyboardEvent<HTMLElement>) => {
    if (event.key !== 'Tab') return;
    const focusable = Array.from(event.currentTarget.querySelectorAll<HTMLElement>('button,[href],input,select,textarea,[tabindex]:not([tabindex="-1"])')).filter((node) => !node.hasAttribute('disabled'));
  ...
  if (!items.length) return <div className="page-shell animate-enter"><div className="mx-auto flex min-h-[58vh] max-w-xl flex-col items-center justify-center text-center"><span className="flex h-14 w-14 items-center justify-center rounded-2xl border border-paper-200 bg-white text-leaf-700 shadow-card"><Bookmark size={24}/></span><p className="eyebrow mt-6">Saved projects</p><h1 className="mt-2 text-3xl font-bold text-ink-950">Your project library is empty.</h1><p className="mt-3 max-w-md text-sm leading-6 text-ink-500">Save a conversion, image trace, GPS measurement, or plan, or restore a previous JSON backup.</p><button type="button" onClick={() => importRef.current?.click()} className="button-primary focus-ring mt-5"><FileUp size={16}/>Restore backup</button><input ref={importRef} type="file" accept="application/json,.json" onChange={importProjects} className="sr-only"/></div></div>;

  return <div className="page-shell animate-enter"><header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="page-header !mb-0"><p className="eyebrow">Saved projects</p><h1 className="page-title">Keep measurements useful and portable.</h1><p className="page-copy">Open, compare, restore, print, or export your projects. Data stays in this browser and is not encrypted by Napiyo.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => importRef.current?.click()} className="button-secondary focus-ring"><FileUp size={16}/>Restore JSON</button><button type="button" onClick={exportCsv} className="button-secondary focus-ring"><Download size={16}/>CSV</button><button type="button" onClick={exportAll} className="button-primary focus-ring"><FileJson size={16}/>Backup all</button><button type="button" onClick={() => setConfirmClear(true)} className="button-secondary focus-ring text-red-700"><Trash2 size={16}/>Clear local data</button><input ref={importRef} type="file" accept="application/json,.json" onChange={importProjects} className="sr-only"/></div></header>
    ...
    {comparison.length === 2 && <div className="fixed inset-0 z-50 overflow-y-auto bg-ink-950/60 p-4 backdrop-blur-sm sm:p-8" role="presentation"><section onKeyDown={trapFocus} className="mx-auto max-w-4xl rounded-2xl bg-white p-5 shadow-soft sm:p-7" role="dialog" aria-modal="true" aria-labelledby="compare-title"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Project comparison</p><h2 id="compare-title" className="mt-2 text-2xl font-bold text-ink-950">Compare saved areas</h2></div><button ref={compareCloseRef} type="button" onClick={() => setSelected([])} aria-label="Close comparison" className="icon-button focus-ring"><X size={19}/></button></div><div className="mt-6 grid gap-4 md:grid-cols-2">{comparison.map((item) => <Compare key={item.id} item={item}/>)}</div><div className="mt-4 rounded-xl bg-ink-950 p-5 text-white"><p className="text-xs uppercase tracking-wide text-ink-300">Difference</p><p className="numeral mt-2 text-3xl font-semibold">{formatDecimal(Math.abs(comparison[0].sqFt - comparison[1].sqFt))} <span className="text-xs text-ink-300">sq ft</span></p></div></section></div>}
    {confirmClear && <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/60 backdrop-blur-sm sm:items-center sm:p-6" role="presentation"><section onKeyDown={trapFocus} className="w-full max-w-md rounded-t-2xl bg-white p-6 shadow-soft sm:rounded-2xl" role="dialog" aria-modal="true" aria-labelledby="clear-title"><p className="eyebrow text-red-700">Local data reset</p><h2 id="clear-title" className="mt-2 text-2xl font-bold text-ink-950">Delete all {items.length} saved projects?</h2><p className="mt-3 text-sm leading-6 text-ink-600">This clears Napiyo data from localStorage and IndexedDB on this browser. Download a backup first if the projects matter.</p><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button ref={clearCancelRef} type="button" onClick={() => setConfirmClear(false)} className="button-secondary focus-ring">Cancel</button><button type="button" onClick={() => { void onClearAll(); setConfirmClear(false); }} className="button-primary focus-ring !border-red-600 !bg-red-600">Delete all local data</button></div></section></div>}
  </div>;
};
