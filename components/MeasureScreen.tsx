...
const MeasureScreen = ({ onSave, notify }: Props) => {
  ...
  const openImage = (source:string) => {
    ...
    next.onload=()=>{setDimensions({width:next.naturalWidth,height:next.naturalHeight});setImage(source);setScalePoints([]);setBoundary([]);setPixelsPerFoot(null);setSaved(false);setError('');setStep('CALIBRATE');};
    next.onerror=()=>setError('That image could not be loaded. Choose another screenshot.');
    next.src=source;
  };
  ...
  return <div className="page-shell animate-enter">
    ...
    {step==='SOURCE'?<section className="panel flex min-h-72 items-center justify-center p-7 text-center"><label className="flex cursor-pointer flex-col items-center"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-leaf-50 text-leaf-700"><Upload size={25}/></span><span className="mt-5 text-xl font-semibold text-ink-950">Upload a plot screenshot</span><span className="mt-2 max-w-md text-sm leading-6 text-ink-500">Choose a clear top-down image containing one labelled or known distance. Images are processed locally and are not sent to a capture service.</span><span className="button-primary mt-5">Choose image</span><span className="mt-3 text-xs text-ink-400">PNG, JPG, WebP · up to 15 MB</span><input type="file" accept="image/*" onChange={upload} className="sr-only"/></label>{error&&<p className="mt-4 text-sm text-red-700">{error}</p>}</section>:
