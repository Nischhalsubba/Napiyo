...
import { BookmarkPlus, Calculator, Check, Copy, ExternalLink, Info, RotateCcw, Share2, Sparkles, Volume2 } from 'lucide-react';
...
const ConvertScreen = ({ onSave, onVisualize, notify }: ConvertScreenProps) => {
  ...
  const switchMode = (nextMode: InputMode, area = sqFt) => {
    ...
    if (nextMode === 'HILLS') { const v=getHillsBreakdown(area); setHills({ropani:valueText(v.ropani,0),aana:valueText(v.aana,0),paisa:valueText(v.paisa,0),daam:valueText(v.daam)}); }
    if (nextMode === 'TERAI') { const v=getTeraiBreakdown(area); setTerai({bigha:valueText(v.bigha,0),kattha:valueText(v.kattha,0),dhur:valueText(v.dhur)}); }
  ...
  const applyQuickValue = (item: (typeof QUICK_VALUES)[number]) => {
    ...
    setSmartInput(`${item.value} ${UNITS[item.unit].name}`);
    setMode('SMART');
    if (UNITS[item.unit].system === UnitSystem.HILLS) { const v=getHillsBreakdown(area); setHills({ropani:String(v.ropani),aana:String(v.aana),paisa:String(v.paisa),daam:valueText(v.daam)}); }
  };

  const clear = () => { setSmartInput(''); setHills({ropani:'0',aana:'0',paisa:'0',daam:'0'}); setTerai({bigha:'0',kattha:'0',dhur:'0'}); setSquareValue('0'); setPrice('0'); };
  ...
  return <div className="page-shell animate-enter !max-w-[96rem]">
    <header className="page-header max-w-4xl"><p className="eyebrow">Nepal land unit converter</p><h1 className="page-title">Type land area naturally. See every equivalent instantly.</h1><p className="page-copy">Use English, नेपाली digits, compact notation, or manual fields. Napiyo explains what it understood before calculating.</p></header>
    <section className="grid gap-6 xl:grid-cols-[25rem_minmax(0,1fr)] xl:items-start">
      <aside className="panel xl:sticky xl:top-4"><div className="panel-header"><p className="section-title">1. Enter your known area</p><p className="section-copy">Smart input supports all Hill, Terai, and standard units.</p></div><div className="panel-body">
        ...
        {mode==='SMART' && <div className="mt-4"><label className="field-label" htmlFor="smart-area">Area</label><textarea id="smart-area" value={smartInput} onChange={e=>setSmartInput(e.target.value)} rows={3} className="field min-h-24 resize-none text-lg font-semibold" placeholder="Example: 1 ropani 2 aana or २ कट्ठा ५ धुर"/><div className={`mt-3 rounded-xl border p-3 text-sm leading-6 ${parsedSmart.recognized?'border-leaf-200 bg-leaf-50 text-leaf-900':'border-saffron-200 bg-saffron-50 text-ink-700'}`}><span className="font-semibold">Interpreted as:</span> {parsedSmart.interpretation}</div></div>}
        ...
        {mode==='SQUARE' && <div className="mt-4"><label className="field-label">Source unit</label><select className="select-field" value={squareUnit} onChange={e=>{const unit=e.target.value as SquareUnit; setSquareValue(valueText(fromSqFt(sqFt,unit),4)); setSquareUnit(unit);}}>{(['SQFT','SQM','SQYD','ACRE','HECTARE'] as SquareUnit[]).map(id=><option key={id} value={id}>{UNITS[id].name}</option>)}</select><label className="field-label mt-4" htmlFor="square-value">Area</label><input id="square-value" inputMode="decimal" value={squareValue} onChange={(e:ChangeEvent<HTMLInputElement>)=>setSquareValue(e.target.value)} className="field numeral text-2xl font-semibold"/></div>}
        <div className="mt-5 flex gap-3 rounded-xl border border-leaf-100 bg-leaf-50 p-3.5 text-sm leading-6 text-leaf-900"><Info className="mt-0.5 shrink-0 text-leaf-700" size={17}/><p>Examples: “1-2-0-0”, “2 kattha 5 dhur”, “500 m²”, “१ रोपनी ४ आना”.</p></div>
        <div className="mt-5"><p className="field-label">Quick examples</p><div className="flex flex-wrap gap-2">{QUICK_VALUES.map(item=><button key={item.label} type="button" onClick={()=>applyQuickValue(item)} className="focus-ring rounded-lg border border-paper-300 bg-white px-3 py-2 text-xs font-semibold text-ink-600">{item.label}</button>)}</div></div>
        <div className="mt-6 border-t border-paper-200 pt-5"><div className="flex items-center gap-2"><Calculator size={17} className="text-leaf-700"/><p className="font-semibold text-ink-950">Land price calculator</p></div><div className="mt-3 grid grid-cols-[1fr_8rem] gap-2"><input value={price} onChange={e=>setPrice(e.target.value)} inputMode="decimal" className="field numeral" aria-label="Price per unit" placeholder="Price in NPR"/><select value={priceUnit} onChange={e=>setPriceUnit(e.target.value as typeof priceUnit)} className="select-field" aria-label="Price unit">{priceUnits.map(id=><option key={id} value={id}>per {UNITS[id].name}</option>)}</select></div><div className="mt-3 rounded-xl bg-ink-950 p-4 text-white"><p className="text-xs text-ink-300">Estimated total price</p><p className="numeral mt-1 text-2xl font-semibold">NPR {displayNumber(totalPrice)}</p><p className="mt-1 text-xs text-ink-300">Planning estimate only. Registration, tax, and brokerage are not included.</p></div></div>
      </div></aside>
      <main className="space-y-5"><section className="overflow-hidden rounded-2xl border border-ink-900 bg-ink-950 text-white shadow-card"><div className="grid gap-6 px-5 py-6 sm:px-7 sm:py-7 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-leaf-300">Exact converted area</p><div className="mt-3 flex flex-wrap items-end gap-x-3"><span className="numeral text-4xl font-semibold sm:text-5xl">{displayNumber(sqFt)}</span><span className="pb-1 text-ink-300">square feet</span></div><p className="mt-2 text-sm text-ink-300">{displayNumber(sqM,3)} square metres</p></div><div className="flex flex-wrap gap-2"><ActionButton onClick={saveCalculation} disabled={!hasValue} primary icon={<BookmarkPlus size={17}/>} label="Save"/><ActionButton onClick={()=>onVisualize(sqFt)} disabled={!hasValue} icon={<ExternalLink size={17}/>} label="Planner"/><ActionButton onClick={speak} disabled={!hasValue} icon={<Volume2 size={17}/>} label="Listen"/><ActionButton onClick={share} disabled={!hasValue} icon={<Share2 size={17}/>} label="Share"/><ActionButton onClick={copySummary} disabled={!hasValue} icon={copied?<Check size={17}/>:<Copy size={17}/>} label={copied?'Copied':'Copy'}/></div></div><div className="grid gap-px bg-white/10 md:grid-cols-2"><BreakdownCard title="Hill breakdown" compact={formatHills(sqFt)} words={formatHillsWords(sqFt)}/><BreakdownCard title="Terai breakdown" compact={formatTerai(sqFt)} words={formatTeraiWords(sqFt)}/></div></section>
        <section className="panel overflow-hidden"><div className="panel-header flex items-center justify-between gap-3"><div><p className="section-title">2. All converted units</p><p className="section-copy">Every individual equivalent updates live.</p></div><button type="button" onClick={()=>setNepaliDigits(v=>!v)} className="button-secondary focus-ring !min-h-9 !px-3">{nepaliDigits?'123':'१२३'}</button></div><div className="grid gap-px bg-paper-200 lg:grid-cols-3"><ConversionGroup title="Hill system" items={hillConversions} display={displayNumber}/><ConversionGroup title="Terai system" items={teraiConversions} display={displayNumber}/><ConversionGroup title="Standard units" items={standardConversions} display={displayNumber}/></div></section>
        <div className="rounded-xl border border-saffron-200 bg-saffron-50 px-4 py-3 text-xs leading-5 text-ink-600"><Sparkles size={15} className="mr-2 inline"/>Conversions are exact mathematical relationships. Legal area and boundaries must come from official records.</div>
      </main>
    </section>
  </div>;
};

const AreaField = ({label,value,onChange}:{label:string;value:string;onChange:(value:string)=>void}) => <label className="block"><span className="field-label capitalize">{label}</span><input inputMode="decimal" value={value} onChange={e=>onChange(e.target.value)} className="field numeral text-center text-xl font-semibold"/></label>;
