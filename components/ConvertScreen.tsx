...
type RecognitionEvent = Event & { results: { 0: { 0: { transcript: string } }; length: number }[] };
...
const ConvertScreen = ({ onSave, onVisualize, notify }: ConvertScreenProps) => {
  ...
  const toggleVoice = () => {
    ...
    recognition.onerror = () => { setListening(false); notify('Voice input could not understand that. Try again in a quieter place.'); };
  ...
  return <div className="page-shell animate-enter !max-w-[96rem]">
    <header className="page-header max-w-4xl"><p className="eyebrow">Nepal land unit converter</p><h1 className="page-title">Type or speak land area naturally.</h1><p className="page-copy">Use English, नेपाली digits, voice, compact notation, or manual fields. Napiyo explains what it understood before calculating.</p></header>
    <section className="grid gap-6 xl:grid-cols-[25rem_minmax(0,1fr)] xl:items-start">
      <aside className="panel xl:sticky xl:top-4"><div className="panel-header"><p className="section-title">1. Enter your known area</p><p className="section-copy">Smart input supports Hill, Terai, and standard units.</p></div><div className="panel-body">
