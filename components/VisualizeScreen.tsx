...
const VisualizeScreen = ({ initialArea, initialProject = null, onBack, onSave, notify }: Props) => {
  ...
  return <div className="page-shell animate-enter !max-w-[100rem]">
    ...
    {invalidBuildings.length > 0 && <div className="mt-5 flex items-start gap-3 rounded-xl border border-error-200 bg-error-50 p-4 text-sm text-error-700" role="alert"><AlertTriangle className="mt-0.5 shrink-0" size={18}/><div><strong>{invalidBuildings.length} building{invalidBuildings.length === 1 ? '' : 's'} extend outside the rectangular plot envelope.</strong><p className="mt-1 text-xs leading-5">Move or resize highlighted buildings before using this plan for space estimates.</p></div></div>}
...
const SitePlan = ({ plan, selectedId, setSelectedId, dragRef, checkpoint, setBuildings, outOfBounds }: { plan:PlannerProject; selectedId:string|null; setSelectedId:(id:string)=>void; dragRef:React.MutableRefObject<{id:string;startX:number;startY:number;originX:number;originY:number}|null>; checkpoint:()=>void; setBuildings:React.Dispatch<React.SetStateAction<PlanBuilding[]>>; outOfBounds:(building:PlanBuilding)=>boolean }) => {
  ...
  return <svg id="napiyo-site-plan" viewBox={`0 0 ${width} ${height}`} tabIndex={0} className="h-auto w-full select-none focus:outline-none focus:ring-4 focus:ring-leaf-300" onKeyDown={keyboardMove} onPointerMove={move} onPointerUp={()=>{dragRef.current=null;}} onPointerLeave={()=>{dragRef.current=null;}} role="application" aria-label="Editable proportional site plan. Select a building and use arrow keys to move it."><defs><pattern id="plan-grid" width={Math.max(scale*5,20)} height={Math.max(scale*5,20)} patternUnits="userSpaceOnUse"><path d={`M ${Math.max(scale*5,20)} 0 L 0 0 0 ${Math.max(scale*5,20)}`} fill="none" stroke={BORDER} strokeWidth="1"/></pattern></defs><rect width={width} height={height} rx="28" fill={plan.showGrid?'url(#plan-grid)':'#fafaf8'}/><polygon points={polygon} fill={PRIMARY_LIGHT} stroke={PRIMARY} strokeWidth="5"/>{plan.buildings.map((building)=>{const rw=(building.rotation===90?building.depth:building.width)*scale;const rh=(building.rotation===90?building.width:building.depth)*scale;const x=ox+building.x*scale,y=oy+building.y*scale;const active=selectedId===building.id;const invalid=outOfBounds(building);return <g key={building.id} onPointerDown={(event)=>{event.currentTarget.setPointerCapture(event.pointerId);checkpoint();setSelectedId(building.id);dragRef.current={id:building.id,startX:event.clientX,startY:event.clientY,originX:building.x,originY:building.y};}} className="cursor-move"><rect x={x} y={y} width={rw} height={rh} rx="8" fill={invalid?'#fff1f0':active?PRIMARY:'#ffffff'} stroke={invalid?'#b42318':PRIMARY} strokeWidth={active||invalid?5:3}/><text x={x+rw/2} y={y+rh/2} textAnchor="middle" dominantBaseline="middle" fontSize="18" fontWeight="700" fill={invalid?'#8a1c13':active?'white':PRIMARY}>{building.name}</text></g>;})}<text x="500" y="35" textAnchor="middle" fontSize="18" fontWeight="700" fill={TEXT}>NORTH: {plan.northSide.toUpperCase()}</text><text x="500" y="675" textAnchor="middle" fontSize="16" fill={MUTED}>Road {formatDecimal(plan.roadWidth)} ft · {plan.roadSide} edge</text>{plan.showDimensions&&<><text x="500" y={oy-18} textAnchor="middle" fontSize="16" fontWeight="700" fill={TEXT}>{formatDecimal(plan.frontage)} ft</text><text x={ox-28} y="350" textAnchor="middle" fontSize="16" fontWeight="700" fill={TEXT} transform={`rotate(-90 ${ox-28} 350)`}>{formatDecimal(plan.depth)} ft</text></>}</svg>;
};
...
const Toggle=({active,label,onClick}:{active:boolean;label:string;onClick:()=>void})=><button type="button" onClick={onClick} className={`focus-ring rounded-lg px-3 py-2 text-xs font-semibold ${active?'bg-leaf-700 text-white':'border border-paper-300 bg-white text-ink-600'}`}>{label}</button>;
