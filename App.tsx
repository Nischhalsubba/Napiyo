...
import { hydrateItems, loadItems, saveItems } from './lib/storage';
import { SavedItem, ViewState } from './types';

const NAV_ITEMS: { id: Exclude<ViewState, 'visualize'>; label: string; mobileLabel: string; icon: typeof Calculator; mobile?: boolean }[] = [
...
const App = () => {
  ...
  useEffect(() => { void hydrateItems().then((items) => { if (items.length) setSavedItems(items); }); }, []);
  ...
  const navigate = (view: ViewState) => {
    ...
    requestAnimationFrame(() => document.getElementById('main-content')?.scrollTo({ top: 0, behavior: 'smooth' }));
  };
  const persist = (next: SavedItem[]) => { const saved = saveItems(next); if (saved) setSavedItems(next); return saved; };
  const handleSave = (item: SavedItem): boolean => persist([item, ...savedItems]);
  const handleImport = (items: SavedItem[]): number => {
    const existing = new Set(savedItems.map((item) => item.id));
    const fresh = items.map((item) => existing.has(item.id) ? { ...item, id: crypto.randomUUID(), title: `${item.title} (imported)` } : item);
  ...
  return <div className="app-shell pb-20 text-ink-950 md:pb-0">
    ...
    <nav className="safe-bottom fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-paper-200 bg-white/94 px-2 pt-2 backdrop-blur-xl md:hidden" aria-label="Bottom navigation">{mobileItems.map((item) => { const Icon = item.icon; const active = activeView === item.id; return <button key={item.id} type="button" onClick={() => navigate(item.id)} aria-current={active ? 'page' : undefined} className={`focus-ring flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[11px] font-semibold ${active ? 'bg-leaf-100 text-leaf-800 ring-1 ring-leaf-200' : 'text-ink-500'}`}><Icon size={19}/>{item.mobileLabel}</button>; })}</nav>
