...
const App = () => {
  ...
  return <div className="app-shell pb-20 text-ink-950 md:pb-0">
    <header className="app-header z-40 border-b border-paper-200/80 bg-white/82 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[96rem] items-center justify-between px-4 sm:px-6 lg:px-8"><button type="button" onClick={() => navigate('convert')} className="focus-ring flex items-center gap-3 rounded-xl text-left"><span className="brand-mark flex h-10 w-10 items-center justify-center rounded-2xl text-white shadow-card"><LandPlot size={20}/></span><span><span className="block text-base font-bold text-ink-950">Napiyo<span className="brand-accent">.</span></span><span className="hidden text-xs font-medium text-ink-500 sm:block">Land measurements, made readable</span></span></button>
      ...
      <div className="hidden items-center gap-2 md:flex"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${online ? 'bg-success-50 text-success-700' : 'bg-saffron-50 text-saffron-700'}`}>{online ? 'Online' : 'Offline'}</span><AccessibilityButtons/><a href="https://github.com/Nischhalsubba/Napiyo" target="_blank" rel="noreferrer" className="icon-button focus-ring" aria-label="View source code"><Code2 size={18}/></a></div>
    ...
    {pendingDelete && <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/48 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="presentation"><section className="w-full max-w-md rounded-t-2xl bg-white p-6 shadow-soft sm:rounded-2xl" role="dialog" aria-modal="true" aria-labelledby="delete-title"><p className="eyebrow !text-error-500">Remove saved project</p><h2 id="delete-title" className="mt-2 text-2xl font-bold text-ink-950">Delete “{pendingDelete.title}”?</h2><p className="mt-3 text-sm leading-6 text-ink-600">This removes the project from this browser. Export a backup first if you may need it later.</p><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button ref={cancelDeleteRef} type="button" onClick={() => setPendingDelete(null)} className="button-secondary focus-ring">Cancel</button><button type="button" onClick={confirmDelete} className="button-primary focus-ring !border-error-500 !bg-error-500 hover:!border-error-700 hover:!bg-error-700">Delete project</button></div></section></div>}
  </div>;
};
